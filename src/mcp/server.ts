...
server.registerTool(
  ...
  async () => json({ workspace: WORKSPACE, projects: scanProjects(WORKSPACE) }),
);
...
server.registerTool(
  ...
  async ({ project }) => json({ project, subagents: fragmentsByType(project, "persona") }),
);
...
server.registerTool(
  ...
  async ({ project }) => json({ project, plans: fragmentsByType(project, "spec") }),
);
